...
async function run() {
    try {
        ...
        const cafeCollection = client.db("cafeDB").collection("menu");
        ...
        const verifyJWT = (req, res, next) => {
            ...
            jwt.verify(token, process.env.ACCESSJWT, (err, decoded) => {
                if (err) {
        ...
        app.post('/jwt', (req, res) => {
            const user = req.body;
            const token = jwt.sign(user, process.env.ACCESSJWT, { expiresIn: "1h" })
            res.send({ token })
        })
        ...
        app.get("/carts", verifyJWT, async (req, res) => {
            ...
            if (email !== decodedEmail) {
                return res.status(403).send({ error: true, message: "Provide Access" })
            }
            const query = { email: email }
...
app.listen(port, () => {
    console.log(`Restaurants on port${port}`)
})
